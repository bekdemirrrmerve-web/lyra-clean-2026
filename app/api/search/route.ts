...
const features = [
  {
    ...
    desc: "Gemini Search ile web destekli araştırma yapar.",
  },
  {
    icon: "✎",
    title: "İçerik Üretme",
    desc: "Hook, başlık ve teleprompter hazırlar.",
  },
  {
    icon: "▰",
    title: "Ders Modu",
    desc: "Konu anlatır, test üretir, yanlış açıklar.",
  },
  {
    icon: "▧",
    title: "Görsel Üretme",
    desc: "Görsel promptu ve konsept hazırlar.",
  },
  {
    icon: "◌",
    title: "Görselle Okut",
    desc: "Görsel, belge ve ekranları analiz eder.",
  },
  {
    icon: "▤",
    title: "PDF Özeti",
    desc: "PDF içeriğini özetler ve not çıkarır.",
  },
  {
    icon: "≋",
    title: "Canlı Mod",
    desc: "Sesli ve görüntülü konuşma alanı.",
  },
];
...
function createOfflineAnswer(question: string) {
  ...
  if (
    ...
    return "Buradayım kanka. Şu an API’siz Lyra modundayım. Yazdığını okuyabilir, temel cevap verebilir, sesli konuşabilir ve canlı modda avatar videosunu oynatabilirim. Güncel araştırma için Online Araştırma moduna geçmen yeterli.";
  }
  ...
  if (
    ...
    return "Evet kanka, çalışıyorum. API’siz modda temel cevap veririm; Online Araştırma modunda /api/search üzerinden Gemini Search route’unu kullanırım. Eğer API çalışmazsa seni boş bırakmam, yine offline moda düşerim.";
  }
  ...
  if (
    ...
    return "Bunun için Online Araştırma moduna geçmelisin kanka. O modda /api/search route’una bağlanıp Gemini Search ile web destekli cevap almayı denerim. Şu an API’siz moddaysan canlı internet taraması yapamam.";
  }
  ...
  if (
    ...
    return `Bunu içerik formatına çevirebiliriz kanka.

HOOK:
“Bunu çoğu kişi yanlış biliyor ama işin mantığı aslında çok daha basit.”

GİRİŞ:
Bugün konuyu sade şekilde açıklıyoruz. Önce problemi netleştiriyoruz, sonra doğru çözüm yolunu seçiyoruz.

GELİŞME:
Burada önemli olan ezbere konuşmak değil; neden-sonuç ilişkisini kurmak. Yani hangi bilgi, hangi durumda işe yarar onu anlamak.

CTA:
Kaydet, sonra tekrar bakarsın.`;
  }
  ...
  if (
    ...
    return `Kozmetik formül mantığıyla ilerlersek önce şunları netleştiririz:

1. Ürün tipi: tonik, serum, krem, jel, şampuan
2. Hedef: nem, bariyer, leke, akne, parlaklık, saç bakımı
3. Baz sistem: su fazı, yağ fazı, emülgatör, kıvam verici
4. Aktifler: niacinamide, panthenol, hyaluronic acid, allantoin gibi
5. pH aralığı
6. Koruyucu sistem
7. Stabilite kontrolü

Kesin formül için ürün tipini, hedefini ve 100 g mı 100 ml mi istediğini yazman yeterli.`;
  }
  ...
  if (
    ...
    return `Tamam kanka, bunu ders modu gibi çalışırız:

1. Konuyu sade anlatırım
2. Örneklerle açıklarım
3. 5 soruluk mini test hazırlarım
4. Yanlış cevapların neden yanlış olduğunu anlatırım
5. En sona kısa tekrar notu çıkarırım

Bana konuyu yazman yeterli.`;
  }

  if (q.includes("pdf") || q.includes("özet") || q.includes("belge")) {
    return "PDF özeti için dosya okuma altyapısının ayrıca bağlanması gerekir. Şimdilik metni buraya yapıştırırsan özetleyebilirim. Sonraki aşamada PDF yükleme ve özetleme modunu ayrı bağlarız.";
  }
  ...
  if (
    ...
    return `Görsel promptu için temel şablon şöyle olabilir:

“Modern, beyaz-gümüş tonlarda, premium yapay zeka asistan arayüzü, cam efektli kartlar, yuvarlak butonlar, soft ışık, minimal ve teknolojik görünüm, yüksek kaliteli UI mockup.”

Bunu Lyra, InciLab, kozmetik laboratuvarı veya içerik üretici odası gibi istediğin konsepte göre özelleştirebilirim.`;
  }

  return "Bunu aldım kanka. Şu an API’siz moddaysam genel bilgi ve şablon mantığıyla cevap veririm. Güncel web araştırması gerekiyorsa Online Araştırma moduna geç; o zaman /api/search üzerinden araştırma yapmayı denerim.";
}

async function askDeepSearch(message: string) {
  ...
  if (!res.ok || !data?.answer) {
    throw new Error(data?.error || "Araştırma altyapısı cevap vermedi.");
  }
    ...
    try {
      ...
      if (aiMode === "local") {
        try {
          ...
          answer =
            "Local AI şu an cevap vermedi kanka. Ollama açık değilse bu normal. API’siz moda düşüyorum:\n\n" +
      ...
      if (aiMode === "online") {
        try {
          answer = await askDeepSearch(clean);
        } catch {
          answer =
            "Online Araştırma bağlantısı gelmedi kanka. /api/search route’u, Gemini kotası veya GEMINI_API_KEY tarafında sorun olabilir. Yine de API’siz modla cevaplıyorum:\n\n" +
            createOfflineAnswer(clean);
        }
      }
      ...
      <aside className="sidebar">
        ...
        <div className="sidebar-bottom">
          ...
          <div className="usage-card">
            ...
            <p>Online mod /api/search kullanır.</p>
          </div>
        ...
        <section className="chat-panel">
          {messages.length === 0 ? (
            <div className="empty-state">
              <b>Lyra hazır.</b>
              <p>
                Araştırmak istediğin şeyi yaz. Online Araştırma modu açıksa
                /api/search üzerinden Gemini Search route’u denenir.
              </p>
            </div>
          ) : (
          ...
          {messages.length === 0 && loading && (
            <div className="message-scroll loading-start">
              <div className="msg-row lyra-row">
                <div className="bubble lyra">
                  <p>Online araştırma deneniyor...</p>
                </div>
              </div>
            </div>
          )}
          ...
          <p className="prompt-hint">
            {aiMode === "offline"
              ? "API’siz mod açık. Lyra temel cevap verir, sesli konuşur."
              : aiMode === "local"
                ? "Local AI modu açık. Ollama açıksa bilgisayarındaki modelle cevap verir."
                : "Online Araştırma modu açık. Gemini Search route’u denenir."}
          </p>
          ...
          <div className="input-box">
            ...
            <input
              ...
              placeholder="Araştırmak istediğin şeyi yaz..."
            />
