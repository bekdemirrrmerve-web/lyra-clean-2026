...
function extractAudioFromLiveMessage(message: any): string[] {
  const parts =
  ...
  return parts
    .map((part: any) => {
      return (
        ...
        part?.inlineData?.inlineData?.data ||
        ""
      );
    })
    .filter(Boolean);
...
export default function LiveGeminiPage() {
  ...
  const startMic = async () => {
    try {
      ...
      setStatus("Hata");
      addLog("Mikrofon açılamadı: " + (error?.message || "hata"));
    }
  };
  ...
  const stopMic = () => {
    try {
      ...
      setIsMicOn(false);
      if (isConnected) setStatus("Bağlandı");
